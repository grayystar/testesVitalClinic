...
/**
 * Gera um código numérico aleatório com o número de dígitos configurado
 * (padrão: 5), usando random_int (fonte criptograficamente segura).
 * Zeros à esquerda são preservados (ex.: "00417").
 */
function generate_numeric_code(int $length = 5): string
{
    $code = '';
    for ($i = 0; $i < $length; $i++) {
...
/**
 * Cria um novo código de recuperação de senha para o usuário, invalida
 * quaisquer códigos anteriores ainda válidos (evita múltiplos códigos
 * ativos ao mesmo tempo) e devolve o código em texto puro — que só
 * existe nesta chamada, para ser enviado por e-mail. O banco armazena
 * apenas o hash (nunca o código em si).
 */
function create_password_reset_code(int $userId): string
{
    $length = (int) (config('rules.password_reset_code_length') ?: 5);
    $minutes = (int) (config('rules.password_reset_expires_minutes') ?: 15);
    $code = generate_numeric_code($length);

    db_transaction(function () use ($userId, $code, $minutes): void {
        // Invalida (consome) qualquer código anterior ainda pendente,
        // para que só o código mais recente seja válido.
        $stmt = db()->prepare(
            'UPDATE password_resets SET consumed_at = ? WHERE user_id = ? AND consumed_at IS NULL'
        );
        $stmt->execute([now_sql(), $userId]);

        repository_append('password_resets', [
            'user_id' => $userId,
            'code_hash' => password_hash($code, PASSWORD_DEFAULT),
            'expires_at' => (new DateTime())->modify('+' . $minutes . ' minutes')->format('Y-m-d H:i:s'),
            'attempts' => 0,
            'consumed_at' => null,
            'created_at' => now_sql(),
        ]);
...
/**
 * Verifica o código informado pelo usuário contra o último código
 * pendente gerado para ele.
 *
 * Regras de segurança:
 *  - código expira após `password_reset_expires_minutes`;
 *  - cada código admite no máximo `password_reset_max_attempts`
 *    tentativas erradas antes de ser bloqueado (o usuário precisa
 *    solicitar um novo código);
 *  - comparação feita com password_verify()/hash, nunca em texto puro;
 *  - ao validar com sucesso, o código é imediatamente consumido
 *    (uso único) para não permitir reaproveitamento.
 */
function verify_password_reset_code(int $userId, string $code): bool
{
    $maxAttempts = (int) (config('rules.password_reset_max_attempts') ?: 5);

    return db_transaction(function () use ($userId, $code, $maxAttempts): bool {
        $stmt = db()->prepare(
        ...
        if (new DateTime($reset['expires_at']) < new DateTime()) {
            return false;
        }

        if (!password_verify($code, $reset['code_hash'])) {
            repository_replace('password_resets', (int) $reset['id'], [
...
/**
 * Define a nova senha do usuário (já com o código de recuperação
 * validado por verify_password_reset_code). A senha é sempre
 * persistida com password_hash() — nunca em texto puro.
 */
